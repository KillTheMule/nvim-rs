  ...
  W: AsyncWrite + Send + Sync + Unpin + 'static,
{
  pub async fn list_bufs(&self) -> Result<Vec<Buffer<W>>, Box<CallError>> {
    Ok(self.call("nvim_list_bufs", call_args![]).await?
      .map(|val| {
  ...
  pub async fn get_current_win(&self) -> Result<Window<W>, Box<CallError>> {
    Ok(self.call("nvim_get_current_win", call_args![])
      .await?
  ...
  pub async fn list_tabpages(
    ...
    Ok(self.call("nvim_list_tabpages", call_args![]).await?
      .map(|val| {
        if let Value::Array(arr) = val {
          arr
            .into_iter()
  ...
  pub async fn get_current_tabpage(
    ...
    Ok(self.call("nvim_get_current_tabpage", call_args![])
      .await?
